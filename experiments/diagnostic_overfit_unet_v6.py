...
"""
Diagnostic overfit-test v5: Residual Dense Restoration Network, not U-Net.

Why this version:
- CAD lines are thin.
- U-Net downsampling can destroy line-level information.
- For JPEG artifact removal / deblurring / super-resolution-like restoration,
  a full-resolution residual CNN is often a better baseline than U-Net.
...
CONFIG = {
    ...
    "task_name": "diagnostic_overfit_rdn_no_unet_v5",
    ...
    # Model: no downsampling, preserve CAD line geometry.
    "channels": 96,
    "num_rdb": 8,
    "growth_channels": 48,
    "layers_per_rdb": 5,
    ...
    "white_threshold": 245,
    "save_dir": "overfit_rdn_v5_outputs",
    "seed": 42,
    "log_every": 10,
}
...
# =========================================================
# MODEL: RESIDUAL DENSE NETWORK, FULL RESOLUTION
# =========================================================
class DenseLayer(nn.Module):
    def __init__(self, in_ch: int, growth_ch: int):
        super().__init__()
        self.net = nn.Sequential(
            nn.Conv2d(in_ch, growth_ch, 3, padding=1),
...
class ResidualDenseBlock(nn.Module):
    def __init__(self, channels: int, growth_channels: int = 48, num_layers: int = 5, residual_scale: float = 0.2):
        super().__init__()
        ...
        for _ in range(num_layers):
            layers.append(DenseLayer(in_ch, growth_channels))
            in_ch += growth_channels

        self.layers = nn.Sequential(*layers)
        self.lff = nn.Conv2d(in_ch, channels, 1)
...
class RDNRestorer(nn.Module):
    """
    Full-resolution residual dense restoration network.

    No pooling.
    No upsampling.
    No tanh.
    No clamp during training.

    This should preserve thin technical/CAD lines better than U-Net.
    """
    def __init__(
        self,
        in_ch=3,
        ...
        self.body = nn.Sequential(*[
            ResidualDenseBlock(
                ...
                num_layers=layers_per_rdb,
                residual_scale=0.2,
            )
            for _ in range(num_rdb)
        ])

        self.body_fusion = nn.Conv2d(channels, channels, 3, padding=1)
        self.tail = nn.Conv2d(channels, out_ch, 3, padding=1)

        # Near-identity start: correction starts from zero.
...
def restoration_loss(pred, hq, lq, correction, cfg):
    ...
    loss = cfg["w_base"] * base + cfg["w_line"] * line + cfg["w_residual"] * residual

    edge_ref = F.l1_loss(sobel_edges(pred), sobel_edges(hq))

    return loss, {
        "l1": F.l1_loss(pred, hq).detach(),
        "weighted_l1": weighted_l1_metric(pred, hq, cfg["line_mask_weight"]).detach(),
        "residual": residual.detach(),
        "edge_ref": edge_ref.detach(),
    }
...
def main():
    ...
    model = RDNRestorer(
        ...
        layers_per_rdb=cfg["layers_per_rdb"],
    ...
    best_path = os.path.join(cfg["save_dir"], "best_overfit_rdn.pth")
            ...
            torch.save(
                {
                    ...
                    "config": dict(cfg),
                    "architecture": "RDNRestorer_no_downsampling",
                },
                best_path,
            )
            task.upload_artifact("best_overfit_rdn", best_path)
