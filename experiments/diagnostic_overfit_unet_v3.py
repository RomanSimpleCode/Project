...
def restoration_loss(pred: torch.Tensor, target: torch.Tensor):
    """
    ResUNet diagnostic loss v3.

    Main objective:
    - Charbonnier: smoother and more robust than plain L1.
    - Line-weighted Charbonnier: forces thin CAD lines to matter more than white background.
    - Edge is only logged as reference, not optimized directly.
    """
    diff = pred - target
    ...
    loss = 0.25 * base_charb + 0.75 * weighted_charb
    return loss, {
...
def main():
    ...
    for epoch in range(1, cfg["epochs"] + 1):
        ...
        # PSNR is intentionally not logged as a main metric for this task.
